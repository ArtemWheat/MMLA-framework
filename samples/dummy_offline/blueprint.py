"""Blueprint for the built-in dummy_offline case."""
...
from implementations.examples.dummy.predictor import DummyAnalyticsPredictor, DummyValidationPredictor

from .collector import prepare_prediction_inputs
from .manifest import CASE_SLUG, DummyOfflineManifest
...
def make_blueprint(context: "CaseBuildContext") -> CaseBlueprint:
    ...
    def build_factory(manifest: DummyOfflineManifest) -> OrchestratorFactory:
        ...
        async def factory() -> CaseOrchestrator:
            ...
            predictor_service.register(PredictionStage.VALIDATION, DummyValidationPredictor(manifest.predictors.validation))
            predictor_service.register(PredictionStage.ANALYTICS, DummyAnalyticsPredictor(manifest.predictors.analytics))
    ...
    return CaseBlueprint(slug=slug_str, manifest_model=DummyOfflineManifest, build_factory=build_factory)